...
const FeatureStocks = () => {
  ...
  useEffect(() => {
    ...
    if (adminAuthToken) {
      ...
      axios
        .get('https://api-staging.ramufinance.com/api/v1/get-featured-stocks', {
          headers: {
            Authorization: `Bearer ${adminAuthToken}`,
  ...
  const handleDeleteClick = (id) => {
    ...
    if (adminAuthToken) {
      axios
        .delete(
          `https://api-staging.ramufinance.com/api/v1/admin/stocks/delete-featured-stock/${id}`,
          {
            headers: {
              Authorization: `Bearer ${adminAuthToken}`,
              "Content-Type": "application/json",
